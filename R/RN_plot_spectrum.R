...
#'
#' @examples
#' RN_plot_spectrum(desired_RN = c("Sr-90", "Y-90"), rad_type = "B-", photon = FALSE, prob_cut = 0.01)
#' RN_plot_spectrum(desired_RN = c("Co-60", "Ba-137m"), rad_type = NULL, photon = TRUE, prob_cut = 0.015)
#' RN_plot_spectrum(desired_RN = c("Co-60", "Ba-137m"), rad_type = NULL, photon = TRUE, log_plot = 0)
#' RN_plot_spectrum(desired_RN = c("Co-60", "Ba-137m"), rad_type = "G")
#' RN_plot_spectrum(desired_RN = c("Pu-238", "Pu-239", "Am-241"), rad_type = "A", photon = FALSE, prob_cut = 0.01, log_plot = 0)
...
if(dat_set != "B")  spec_df <- spec_df[which(spec_df$prob > prob_cut), ]
  ...
  p <- ggplot2::ggplot(
    data = spec_df,
    ggplot2::aes(E_MeV, prob, color = RN, shape = RN, size = MeV_per_dk)
    ) +
    ggplot2::geom_point(alpha = 0.7, stroke = 3) +
    ggplot2::scale_shape_manual(values = c(0, 5, 7, 9, 12)) +
    ggplot2::ggtitle(paste0(rad_text, " ", spec_text),
                     subtitle = paste0("particle probability > ", prob_cut)) +
    ggplot2::guides(shape = ggplot2::guide_legend(override.aes = list(size = 3)),
           size = ggplot2::guide_legend(override.aes = list(shape = 0, alpha = 0.7, color = "gray", stroke = 1.5)))
