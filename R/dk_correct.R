...
#' my_dks <- dk_correct(RN_select = "Sr-90", 
#'                      date1 = "2009-01-01", 
#'                      date2 = "2019-01-01", 
#'                      num = TRUE)
#' 
#' #   RN_select and time_lapse (random sample)
#' dk_correct(RN_select = base::sample(RadData::ICRP_07.NDX$RN, 1),
#'      time_lapse = 1:10, 
...
# Time-decayed response, A2
  ...
  # Each of these will use the decay formula:
    ...
    if (!is.null(RN_select) & !is.null(date1)) {
      n <- length(date2)
     df <- data.frame("RN" = rep(RN_select, n),
                   "RefValue" = rep(A1, n),
                   "RefDate" = rep(date1, n),
                   "TargDate" = date2,
                   "dk_value" = A2)
  ...
  if ((!is.null(half_life) & !is.null(date1))) {
    n <- length(date2)
    df <- data.frame("half_life" = rep(half_life, n),
                     "RefValue" = rep(A1, n),
                     "RefDate" = rep(date1, n),
                     "TargDate" = date2,
                     "dk_value" = A2)
